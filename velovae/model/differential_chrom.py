...
def differential_dynamics(adata,
    ...
    if len(idx2) == adata.n_obs:
        idx2 = np.where(idx2)[0]
    c = adata_atac.layers['Mc'].copy()
    c = c.A if issparse(c) else c
    u = adata.layers['Mu'].copy()
    u = u.A if issparse(u) else u
    s = adata.layers['Ms'].copy()
    s = s.A if issparse(s) else s
    g1_corrected = model.test(c[idx1], u[idx1], s[idx1], batch=(None if batch_correction else adata.obs[batch_key][idx1]))
    g2_corrected = model.test(c[idx2], u[idx2], s[idx2], batch=(None if batch_correction else adata.obs[batch_key][idx2]))
